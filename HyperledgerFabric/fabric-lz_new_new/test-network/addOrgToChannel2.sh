...
configtxlator compute_update --channel_id channel2 --original config.pb --updated modified_config.pb --output "${ORG_NAME}"_update.pb

configtxlator proto_decode --input "${ORG_NAME}"_update.pb --type common.ConfigUpdate --output "${ORG_NAME}"_update.json

echo '{"payload":{"header":{"channel_header":{"channel_id":"'channel2'", "type":2}},"data":{"config_update":'$(cat "${ORG_NAME}"_update.json)'}}}' | jq . > "${ORG_NAME}"_update_in_envelope.json
...
echo "组织 org1 已签署配置更新"
# 签署配置更新
if [ "$ORG_NUMBER" -gt 4 ]; then
  for ((i=3; i<ORG_NUMBER; i++)); do
    if [ "$i" -eq 4 ]; then
      continue  # 跳过 i 等于 4 的情况
    fi
    ORG_MSP="Org${i}MSP"
    export CORE_PEER_TLS_ENABLED=true
    ...
    export CORE_PEER_ADDRESS=localhost:$((10000 + i * 1000 + 1))
    
    peer channel signconfigtx -f channel-artifacts/"${ORG_NAME}"_update_in_envelope.pb
    echo "组织 ${ORG_MSP} 已签署配置更新"
  done
fi
...
peer channel update -f channel-artifacts/"${ORG_NAME}"_update_in_envelope.pb -c channel2 -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile "${PWD}/organizations/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem"
...
do
  ...
  echo "peer${i}.${ORG_NAME} 已加入 channel2"
done
...
configtxlator compute_update --channel_id channel2 --original config.pb --updated modified_anchor_config.pb --output anchor_update.pb
configtxlator proto_decode --input anchor_update.pb --type common.ConfigUpdate --output anchor_update.json
echo '{"payload":{"header":{"channel_header":{"channel_id":"channel2", "type":2}},"data":{"config_update":'$(cat anchor_update.json)'}}}' | jq . > anchor_update_in_envelope.json
configtxlator proto_encode --input anchor_update_in_envelope.json --type common.Envelope --output anchor_update_in_envelope.pb
cd ..
peer channel update -f channel-artifacts/anchor_update_in_envelope.pb -c channel2 -o localhost:7050 --ordererTLSHostnameOverride orderer.example.com --tls --cafile "${PWD}/organizations/ordererOrganizations/example.com/orderers/orderer.example.com/msp/tlscacerts/tlsca.example.com-cert.pem"
